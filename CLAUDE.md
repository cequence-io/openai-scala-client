...
- **GPT-6 Sol / Luna** (`gpt6SolLuna` = the GPT-5.6 rules): live-verified 2026-09-22 - unlike Astra they accept `reasoning_effort=none` and function tools on chat completions (with `none` only). There is no `gpt-6-terra` (404 on 2026-09-22); an unknown `gpt-6-*` id gets the Sol/Luna rules (`ChatCompletionSettingsConversions.isGpt6Astra` decides)
- **Tool routing for the reasoning-first models**: `chatToolsPreferResponsesAPI(settings, tools)` - on a Responses-capable service (the full `OpenAIService`, sync and typed streamed) tool calls of GPT-6 Astra ALWAYS, and of GPT-5.6 / GPT-6 Sol/Luna unless `reasoning_effort = none`, go through the Responses API so reasoning survives; a chat-only service forces `none` (`gpt5_6ChatTools`), Astra fails fast. The Responses adapter (`toResponsesSettings`) applies its own rules: `responsesReasoningEffort` (`max` kept, `minimal`→`low`, Astra `none`→`low`) and `responsesSamplingUnsupported` (temperature/top_p/top_logprobs dropped on GPT-5.6/6)
- **GPT-6 (Astra)**: Live-verified 2026-09-05 (unchanged 2026-09-22). Reasoning-first like GPT-5.6: all sampling params restricted, `max_tokens→max_completion_tokens`, `reasoning_effort` `max`→`xhigh` on chat completions (Responses-API-only), `minimal`→`low` and `none`→`low` (both rejected by the API). Function tools are NOT accepted on the chat completions API at all, so `createChatToolCompletion` is routed through the Responses API on the full `OpenAIService` (a chat-only service fails fast with an explanatory exception)
