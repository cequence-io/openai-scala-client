...
trait WSRequestHelper extends WSHelper {
  ...
  protected def execPOSTMultipartWithStatus(
    ...
    val (request: StandaloneWSRequest, formData: MultipartFormData, writeable) =
      parseRequest(endPoint, endPointParam, params, fileParams, bodyParams)
  ...
  protected def parseRequest(
    ...
    val writeable: BodyWritable[MultipartFormData] =
      writeableOf_MultipartFormData("utf-8")
    (request, formData, writeable)
  }
